"""Versioned CH-009 evaluator with pre-registered mechanism ablations."""
...
from etas_challenge.phase_coherent_debt_fit import PhaseCoherentDebtEvaluation
from etas_challenge.phase_coherent_debt_fit import PhaseCoherentDebtParameters
...
class PhaseCoherentDebtAblationFitEvaluator:
    """Replay CH-009 with one declared synthesis factor removed."""
    ...
    def evaluate(
        ...
        *,
        ...
        if ablation not in ABLATIONS:
            raise ValueError(f"unknown CH-009 ablation: {ablation}")
        base = self.parent
        ...
        challenger_rates = np.empty(
            len(base.event_days) - base.scoring_event_start, dtype=float
        )
        active_issue_days = 0
        ...
        for day in base.issue_days:
            ...
            if day >= base.scoring_start_day and end > start:
                ...
                for branch, geometry in enumerate(base.grid_geometries):
                    ...
                    phase_acceleration = (
                        np.ones_like(acceleration[branch])
                        if ablation == "acceleration_removed"
                        else acceleration[branch]
                    )
                    phase_frailty = (
                        np.ones_like(frailty)
                        if ablation == "frailty_level_removed"
                        else frailty
                    )
                    coherence_mix = (
                        0.0
                        if ablation == "network_coherence_removed"
                        else phase_parameters.coherence_mix
                    )
